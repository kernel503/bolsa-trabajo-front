<template>
  <v-data-table :headers="headers" :items="listado" class="elevation-1">
    <template v-slot:top>
      <v-toolbar flat>
        <v-toolbar-title>Tipos de conocimiento</v-toolbar-title>
          ...
          <v-card>
              ...
              <v-container>
                <v-row>
                  <v-col cols="12" sm="12" md="12">
                    <v-text-field
                      v-model.trim="editedItem.nombreConocimiento"
                      label="Nombre"
                      @keydown.enter="save"
                      autofocus
                    ></v-text-field>
            ...
            <v-card-actions>
              ...
              <v-btn
                ...
                text
                @click="save"
                :disabled="!editedItem.nombreConocimiento"
              >
                Guardar
              </v-btn>
            </v-card-actions>
...
export default {
  ...
  methods: {
    ...
    deleteItemConfirm() {
      // eslint-disable-next-line no-undef
      axios
        .delete(this.urlBase + this.editedItem.idTipoConocimiento)
        .then((response) => {
          if (response.data.statusCode === 200) {
    ...
    save() {
      if (!this.editedItem.nombreConocimiento) {
        this.$store.commit("SHOW_NOTIFICATION", {
