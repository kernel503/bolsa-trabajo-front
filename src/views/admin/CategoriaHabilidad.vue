<template>
  <v-data-table :headers="headers" :items="listado" class="elevation-1">
    <template v-slot:top>
      <v-toolbar flat>
        <v-toolbar-title>Categoria Habilidad</v-toolbar-title>
          ...
          <v-card>
            ...
            <v-card-actions>
              ...
              <v-btn
                ...
                text
                @click="save"
                :disabled="!editedItem.informacion"
              >
                Guardar
              </v-btn>
            </v-card-actions>
...
export default {
  ...
  methods: {
    ...
    deleteItemConfirm() {
      // eslint-disable-next-line no-undef
      axios
        .delete(this.urlBase + this.editedItem.idCategoriaHabilidad)
        .then((response) => {
          if (response.data.statusCode === 200) {
    ...
    save() {
      if (!this.editedItem.informacion) {
        this.$store.commit("SHOW_NOTIFICATION", {
